...
class pinyin {
	...
	public static function first($string){
		return self::out($string,true,'first');
	}
	...
	public static function full($string){
		return self::out($string,true,'full'); 
	}
	
	/**
	 *输出
	 */
	private static function out($string, $utf8 = true,$type='') {
		$string = ($utf8 === true) ? iconv('utf-8', 'gbk//IGNORE', $string) : $string;
		if (self::$pinyinArr == NULL) {
			self::$pinyinArr = self::pinyin_code();
		}
		$num = strlen($string);
		$pinyin = '';
        for ($i=0; $i < $num; $i++) {
			...
			if ($temp > 0 && $temp < 160) {
				...
				$py= self::$pinyinArr[$j][0]; 
				if($type=='first'){
					$pinyin .=mb_substr($py,0,1);
				}else{
					$pinyin .=$py;
				}
			}
	 ...
	 * @return array
	 */
	private static function pinyin_code() {
		return array(
            array("a",-20319),
            array("ai",-20317),
            array("an",-20304),
