 ...
 * 邮件处理类，可以发送邮件           *
 * *******************************************************************************
 	$smtpemailto = "hait88@126.com";//发送给谁 
	$mailsubject = "okokook";//邮件主题 
	$mailbody = "成功的东西";//邮件内容 
	$mail = new email('smtp.126.com',25,true,'用户名','密码',"hait88@126.com");//这里面的一个true是表示使用身份验证,否则不使用身份验证. 
	$mail->sendmail($smtpemailto, $mailsubject, $mailbody, "TXT"); 
 */
class email 
{ 
    /* Public Variables */ 
    ...
    /* Constractor */ 
    function __construct($relay_host = "", $smtp_port = 25,$auth = false,$user,$pass,$fromEmail) {
        $this->debug = FALSE; 
